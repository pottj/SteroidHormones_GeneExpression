...
#' # Load data ####
#' ***
load("../results/02_MVMR_IVW.RData")
load("../temp/eqtlgen_harmonized.RData")
load("../temp/BMI_TT_IVs.RData")
...
MR_results[,exposure2 := paste(exposure,setting,sep="_")]

#' # Hier FDR ####
#' ***
myFDR = addHierarchFDR(pvalues = MR_results$pval_IVW, categs = MR_results$exposure2)
...
MR_results[, table(hierFDR,exposure2)]
MR_results[,min(P.Value.adj1),by=exposure2]
MR_results[,min(pval_IVW),by=exposure2]
MR_results[,table(P.Value.adj1<0.05,exposure2)]
MR_results[,table(pval_IVW<0.05,exposure2)]
MR_results[hierFDR==T,outcome]

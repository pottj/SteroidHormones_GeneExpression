...
#' # Introduction ####
#' ***
#' In this script, I will load the total testosterone data from Ruth et al.(2020), and filter for 
#' 
...
dumTab1 = foreach(i=1:dim(ToDoList)[1])%do%{
  ...
  # load data
  myfn1 = paste0(path_Ruth_TT_2020,myRow$filenames)
  erg1 = fread(myfn1)
...
save(myTab_complete,file ="../temp/02_TT_unfiltered.RData")
...
save(myTab_filtered,file ="../temp/02_TT_filtered_LIFE.RData")
...
filt = myTab_sameSNPSet$cyto == "04q13.2" & myTab_sameSNPSet$rsID == "rs4694202"
table(filt)
myTab_sameSNPSet = myTab_sameSNPSet[!filt, ]

#' # Save data ####
#' ***
save(myTab_bestPerSet,file ="../results/02_TT_bestSNPperSetting.RData")
save(myTab_sameSNPSet,file = "../results/02_TT_sharedOrIndepSNPs.RData")
save(myColoc,file = "../results/02_TT_Coloc.RData")

table(is.element(myTab_bestPerSet$dumID2,pvar$ID))
table(is.element(myTab_sameSNPSet$dumID2,pvar$ID))

write.table(myTab_bestPerSet[setting=="women",c(18,6,9)],file = "../temp/02_TT_ScoreWeights_Women_best.txt",
            col.names = T,row.names = F,quote = F)
write.table(myTab_bestPerSet[setting=="men",c(18,6,9)],file = "../temp/02_TT_ScoreWeights_Men_best.txt",
            col.names = T,row.names = F,quote = F)

write.table(myTab_sameSNPSet[setting=="women",c(18,6,9)],file = "../temp/02_TT_ScoreWeights_Women_same.txt",
            col.names = T,row.names = F,quote = F)
write.table(myTab_sameSNPSet[setting=="men",c(18,6,9)],file = "../temp/02_TT_ScoreWeights_Men_same.txt",
            col.names = T,row.names = F,quote = F)

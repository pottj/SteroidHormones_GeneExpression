#' ---
#' title: "PGS - Analysing PGS and Visualizing results restricted to pathway region"
...
for(j in 1:length(studies)){
  ...
  myScoreFiles = list.files(path="../results/02_PGS_05_PLINK_PGS_pathway//", pattern="sscore")
  ...
  for(i in 1:length(myScoreFiles)){
    ...
    # Step 4: load score
    myScore = fread(paste0("../results/02_PGS_05_PLINK_PGS_pathway/",myScoreFiles[i]))
  ...
  # Step 7: save object 
  save(SamplesAsImputed, file = paste0(path_LIFEprepped,"genetics/",studies[j],"_PGS_pathway.RData"))
...
#' Only strong instruments for testosterone, and for one E2 setting in men (p-value range between 0 and 0.001). 
#' 
...
#' In the Heart study there are no strong instruments. 
#' 
#' # Save PGS results ####
#' ***
...
save(PGS.result,file = "../results/02_PGS_06_PGSresults_pathway.RData")
write.table(PGS.result, file = "../results/02_PGS_06_PGSresults_pathway.txt",quote = F,sep="\t",row.names = F,col.names = T,dec = ".")
...
for(i in 1:dim(ToDoList)[1]){
  ...
  ggsave(paste0("../results/02_PGS_06_PGS_barplots/",filename,".png"), height = 7, width = 7)

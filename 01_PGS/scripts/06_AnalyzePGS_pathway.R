...
for(j in 1:length(studies)){
  ...
  myScoreFiles = list.files(path="../results/05_PLINK_PGS_pathway//", pattern="sscore")
  ...
  for(i in 1:length(myScoreFiles)){
    ...
    # Step 4: load score
    myScore = fread(paste0("../results/05_PLINK_PGS_pathway/",myScoreFiles[i]))
...
save(PGS.result,file = "../results/06_PGSresults_pathway.RData")
write.table(PGS.result, file = "../results/06_PGSresults_pathway.txt",quote = F,sep="\t",row.names = F,col.names = T,dec = ".")
...
for(i in 1:dim(ToDoList)[1]){
  ...
  ggsave(paste0("../results/06_PGS_barplots/",filename,".png"), height = 7, width = 7)

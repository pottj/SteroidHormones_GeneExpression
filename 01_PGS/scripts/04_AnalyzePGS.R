...
for(j in 1:length(studies)){
  ...
  myScoreFiles = list.files(path="../results/03_PLINK_PGS/", pattern="sscore")
  ...
  for(i in 1:length(myScoreFiles)){
    ...
    # Step 4: load score
    myScore = fread(paste0("../results/03_PLINK_PGS/",myScoreFiles[i]))
...
save(PGS.result,file = "../results/04_PGSresults.RData")
write.table(PGS.result, file = "../results/04_PGSresults.txt",quote = F,sep="\t",row.names = F,col.names = T,dec = ".")
...
for(i in 1:dim(ToDoList)[1]){
  ...
  ggsave(paste0("../results/04_PGS_barplots/",filename,".png"), height = 7, width = 7)
